        ...
        fn find_dll(dir: &std::path::Path) -> Option<std::path::PathBuf> {
            ...
            if let Ok(entries) = std::fs::read_dir(dir) {
                for e in entries.flatten() {
                    let p = e.path();
                    if p.is_dir() {
                        if let Some(found) = find_dll(&p) {
                            return Some(found);
                        }
                    } else if let Some(n) = p.file_name().and_then(|s| s.to_str()) {
                        if n.to_ascii_lowercase().contains("dotlin_runtime")
                            && n.to_ascii_lowercase().ends_with(".dll")
                        {
                            return Some(p);
                        }
                    }
...
fn test_basic_hello_world_codegen() {
    ...
    let _ = std::fs::write(&src, "fun main() {\n  println(\"Hello\")\n}\n");
    ...
    assert!(
        ...
        "basic hello world failed to run: stdout={} stderr={}",
